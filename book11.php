...
<div class="container">
  ...
  <?php     
   $sql = "SELECT * FROM book_time ORDER BY bid DESC";
    ...
    if ($result->num_rows > 0) {
        ...
        ?>
            ...
            <td><input class="top-10" type="radio" id="vehicle<?php echo $row["bid"];?>" name="time" value="<?php echo $row["bid"];?>">
                <label for="vehicle<?php echo $row["bid"];?>"> <?php echo $row["booking_time"];?></label></td>
...
<script>
  function check_slot_Availability(slot) {
    ...
    alert(slot);
		...
		jQuery.ajax({
				url: "<?php echo SITE_PATH;?>admin/check_slot.php",
				data:
				{
				    book_date:book_date,
				    slot:slot,
				...
				success:function(result){
                     ...
                     if(obj.is_error=='Yes'){
                        document.getElementById("slot-availability-status").innerHTML= obj.is_error + ", " + obj.message; 
